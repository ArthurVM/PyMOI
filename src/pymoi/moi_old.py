...
class MOI(object):
    ...
    def bootstrap(self, n : int=5, sample_fraction : float=0.05, min_k : int=1, max_k : int=5):

        pltbox = []
        gmmbox = []
        sample = self.af_matrix     ## store the sample

        ## bootstrap start
        for i in range(1,n+1):
            sample_indices = list(np.random.choice(len(sample), size=int(len(sample)*sample_fraction), replace=False))
            self.af_matrix = pd.DataFrame(sample.iloc[sample_indices])

            ## run the gmm fit
            print(f"Bootstrap {i}: {len(self.af_matrix)}")
            gmm = self.fit_binomial_mixture_model(min_k=min_k, max_k=max_k)

            ## get responsibilities for each data point
            responsibilities = self._responsibilities(gmm)
            n = len(responsibilities)
            resp_ss = sorted(responsibilities)[:int(n*0.1)]
            print(resp_ss, np.mean(resp_ss))

            pltbox.append(self.plot_baf(gmm))
            gmmbox.append(self.af_matrix)

        self.af_matrix = sample
        return pltbox, gmmbox


    def fit_binomial_mixture_model(self, min_k : int=1, max_k : int=5, n_iter : int=5, responsibility_upper : float=0.1, random_state=None) -> GMM:
